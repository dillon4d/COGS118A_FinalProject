# note: same as Drug_functions.py but had to change the max_iter to 500 for logistic regression
# due to Mushroom dataset being significantly larger
import numpy as np
...
def optimized(weight, K, F, C, D, penalty, X_train, y_train, X_test, y_test,train_dict,test_dict):
    ...
    clf_lr = LogisticRegression(penalty=penalty, C = C, max_iter= 500)
